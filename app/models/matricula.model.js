...
class Matricula {
  ...
  static async create(matricula) {
    const { aluno_id, data_matricula, status } = matricula;
    const [result] = await db.query(
      'INSERT INTO matriculas (aluno_id, data_matricula, status) VALUES (?, ?, ?)',
      [aluno_id, data_matricula, status]
    );
  ...
  static async update(id, matricula) {
    const { aluno_id, data_matricula, status } = matricula;
    await db.query(
      'UPDATE matriculas SET aluno_id = ?, data_matricula = ?, status = ? WHERE id = ?',
      [aluno_id, data_matricula, status, id]
    );
    return { id, ...matricula };
  }
